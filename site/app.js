...
function detail(id) {
  ...
  const person = record.creator ? `${t('originalCreator')}: ${personLabel(record.creator)}` : `${t('sourcePublisher')}: ${personLabel(record.publisher)}`;
  const preview = record.kind === 'research'
  ...
  const researchMeta = record.kind === 'research'
    ? `<br>${t('sourceTitle')}: ${esc(record.source_title || '—')}<br>${t('rights')}: ${esc(record.rights_status || '—')}${record.quality_note ? `<br>${t('qualityNote')}: ${esc(record.quality_note)}` : ''}`
    : '';
