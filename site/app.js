...
const zh = {
  navLibrary: '图鉴', navAgent: '给 Agent', eyebrow: 'GPT IMAGE 2 · 人像处理', heroTitle: '<span class="cjk-line">人像提示词</span><br><em class="cjk-line">集中收录，快速找到。</em>', heroCopy: '一个专注人像处理的 GPT Image 2 提示词专题站。按处理方向与具体场景整理原始提示词，让需要的人更快找到合适的案例。', browse: '浏览全部案例', source: '查看源库 ↗', statCases: '条图鉴记录', statTracks: '个人像方向', statLinks: '条原始来源可回链', principleOneTitle: '原文保留', principleOneText: '已收录原文按来源展示；研究候选会明确标注摘录状态，不把摘要冒充完整提示词。', principleTwoTitle: '来源可追溯', principleTwoText: '每条记录都回链原始来源，并清楚标注发布者、作者核验状态和许可状态。', principleThreeTitle: '为真实人像而设', principleThreeText: '围绕身份保持、自然质感与具有纪念意义的影像处理做筛选。', catalogEyebrow: '图鉴目录', catalogTitle: '从真实需求开始找', catalogCopy: '选择处理方向与具体场景，打开提示词摘录、作者、来源和核验状态。', search: '搜索提示词、场景或发布者', allCategories: '全部处理方向', allSecondary: '全部具体场景', chooseSecondary: '先选择处理方向', allIdentity: '任意身份控制', identityExplicit: '明确锁定身份', identityReference: '以参考图保持身份', identityImplicit: '隐含身份控制', identityNone: '未写身份控制', results: '条记录', reset: '清除筛选', openSource: '打开原始来源 ↗', sortRecent: '按最新收录', sortShortest: '按提示词长度', sourcePublisher: '来源发布者', originalCreator: '已核验原创作者', creditedPerson: '页面列出的作者/署名', provenance: '来源状态', prompt: '原始提示词（未翻译）', researchBadge: '研究候选', researchPrompt: '研究摘录（原文未完整收录）', researchLeadPrompt: '来源线索（未复制正文）', researchPreview: '暂无原帖效果图 · 仅展示来源摘录', sourceTitle: '来源页面', rights: '许可/使用状态', qualityNote: '采集备注', copyPrompt: '复制摘录', task: '处理方向', scenario: '具体场景', identity: '身份保持', noResults: '没有符合条件的记录。', agentEyebrow: '内置 SKILL', agentTitle: '把人像图鉴带进<br><em>Claude Code 和 Codex。</em>', agentCopy: '同一份目录既可浏览，也能让你的 Agent 根据真实人像需求挑选合适的原始提示词，同时保留来源信息。', installLabel: '快速安装', copy: '复制', copied: '已复制', installNote: '安装后直接说：使用 gpt-image-2-portrait-library，为一张旅行人像找一个自然、保留身份的处理方案。', ethicsEyebrow: '署名也是工作流的一部分', ethicsTitle: '好看的结果，不该抹去创作者的来处。', ethicsCopy: '此库区分“发布者”和“已核验原创作者”。如果来源不充分，不把发布账号冒充作者；作者可以随时发起更正或移除。', ethicsLink: '查看署名与更正规则 ↗', footer: '提示词始终保持来源的原始语言。'
};
const en = { ...zh, navLibrary: 'Library', navAgent: 'For agents', eyebrow: 'GPT IMAGE 2 · PORTRAIT EDITING', heroTitle: 'Portrait prompts,<br><em>organized to be found.</em>', heroCopy: 'A focused GPT Image 2 atlas for portrait editing. Browse original prompts by outcome and scenario to find the right case faster.', browse: 'Browse the library', source: 'View source repo ↗', statCases: 'atlas records', statTracks: 'portrait tracks', statLinks: 'records source linked', catalogEyebrow: 'ATLAS CATALOG', catalogTitle: 'Choose the outcome first', catalogCopy: 'Choose an editing outcome and concrete scenario, then open the prompt excerpt, creator, source and verification state.', search: 'Search prompt, scene or publisher', allCategories: 'All editing outcomes', allSecondary: 'All scenarios', chooseSecondary: 'Choose an outcome first', allIdentity: 'Any identity control', identityExplicit: 'Explicit identity lock', identityReference: 'Reference-based identity', identityImplicit: 'Implicit identity lock', identityNone: 'No identity lock', results: 'records', reset: 'Reset filters', openSource: 'Open original source ↗', sortRecent: 'Recently collected', sortShortest: 'Shortest prompt first', sourcePublisher: 'Source publisher', originalCreator: 'Verified original creator', creditedPerson: 'Named / credited person', provenance: 'Provenance', prompt: 'Original prompt (untranslated)', researchBadge: 'Research candidate', researchPrompt: 'Research excerpt (not full source text)', researchLeadPrompt: 'Source lead (full text not copied)', researchPreview: 'No source result image · excerpt only', sourceTitle: 'Source page', rights: 'Rights / usage status', qualityNote: 'Collection note', copyPrompt: 'Copy excerpt', task: 'Outcome', scenario: 'Scenario', identity: 'Identity', noResults: 'No matching records.' };
zh.navWechat = '微信公众号';
...
function cardMarkup(record) {
  const preview = record.kind === 'research'
    ? `<div class="research-preview"><span>RESEARCH</span><strong>${esc(t('researchBadge'))}</strong><small>${esc(t('researchPreview'))}</small></div>`
...
function detail(id) {
  ...
  const preview = record.kind === 'research'
    ? `<div class="detail-research-preview"><span>${esc(t('researchPreview'))}</span></div>`
  ...
  const researchMeta = record.kind === 'research'
    ? `${creditedMeta}<br>${t('sourceTitle')}: ${esc(record.source_title || '—')}<br>${t('rights')}: ${esc(record.rights_status || '—')}${record.quality_note ? `<br>${t('qualityNote')}: ${esc(record.quality_note)}` : ''}`
    : '';
  const canCopy = record.kind !== 'research' || ['short_verbatim_excerpt', 'structure_only'].includes(record.prompt_text_status);
